...
file_data = [
b"""
UEsDBAoAAgAAAO9lmVah+EowFwAAABcAAAAIABwAZGF0YS50eHRVVAkAA0IgSGQsIEhkdXgLAAEE
6AMAAAToAwAAMTIzIGFiYwo0NTYgZGVmCjc4OSBnaGlQSwECHgMKAAIAAADvZZlWofhKMBcAAAAX
AAAACAAYAAAAAAABAAAApIEAAAAAZGF0YS50eHRVVAUAA0IgSGR1eAsAAQToAwAABOgDAABQSwUG
AAAAAAEAAQBOAAAAWQAAAAAA
"""
]
...
def main():
    ...
    # read data from existent file
    system_file_handler = File_Handler("./data.txt", "a+")
    system_file_handler.goto(0)
    lines = system_file_handler.read_file(return_lines=True)
    
    # change to new file
    system_file_handler.change_file_name("./data_2.txt", True)
    ...
    for line in lines:
        system_file_handler.write_file(f"{line}")

    
    system_file_handler.write_file("\n")
    system_file_handler.write_file(f"{new_data[0]}\n")
    system_file_handler.write_file(f"{new_data[1]}\n")
